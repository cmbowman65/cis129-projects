...
def main():
    ...
    while True:
        ...
        print("Last word repeated a randon number of times:\n")
        print(f"{repeat}\n")
        ...
        for k, v in word_dictionary.items():
            k += " "
            print(k.ljust(23, ".") + str(v).rjust(3))
    ...
    print("Thanks for sharing your favorite quote or lyrics!")
...
def last_word_random_repeat(string):
    ...
    return "  ".join([last_word for _ in range(random.randint(1, 16))])
...
def twenty_char_slice(string):
    ...
    if len(string) >= 20:
        num = random.randint(0, (len(string) - 20))
